...
/**
 * The LLM API (1.20.46): hand a ticket up to the editing-teacher/manager
 * tier.
 *
 * A thin wrapper over tickets::escalate() - no new state logic (non-
 * negotiable 4). There is no refer endpoint (design doc: "it cannot
 * judge coordinator suitability") - escalate is the machine's only
 * hand-off, and it is a hand-off to HUMANS, never a hand-off to another
 * automated actor.
 *
 ...
 */
class api_escalate extends external_api {
    /**
    ...
    public static function execute_parameters(): external_function_parameters {
        return new external_function_parameters([
            ...
            'note' => new external_value(PARAM_RAW, 'Why this needs a human above the machine'),
            'noteformat' => new external_value(PARAM_INT, 'Text format of note', VALUE_DEFAULT, FORMAT_PLAIN),
        ]);
    }

    /**
     * Escalate a live ticket.
     *
    ...
    public static function execute(int $ticketid, string $note, int $noteformat = FORMAT_PLAIN): array {
        ...
        $ticket = tickets::escalate($activity, $ticketid, $note, $noteformat, $userid);
