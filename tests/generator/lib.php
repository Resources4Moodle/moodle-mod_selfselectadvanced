...
class mod_selfselectadvanced_generator extends testing_module_generator {
     ...
     */
    public function create_joinrequest($record): stdClass {
        global $DB;

        $record = (object) (array) $record;
        foreach (['activityid', 'userid', 'targetgroupid'] as $field) {
            if (!isset($record->$field)) {
                throw new coding_exception('create_joinrequest requires ' . $field);
            }
        }
        ...
        $request = (object) [
            ...
            'replaceleader' => 0,
            'successorid' => null,
            'status' => $record->status ?? \mod_selfselectadvanced\local\joinrequests::STATUS_REQUESTED,
            'statusinfo' => null,
            'reason' => $record->reason ?? 'Generated by the test fixture',
            'responsenote' => $record->responsenote ?? null,
            'usermodified' => (int) $record->userid,
            'timecreated' => $now,
            'timemodified' => $now,
        ];
