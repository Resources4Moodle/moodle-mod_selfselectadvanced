...
1. **Root filesystem hit 100% mid-gate** (behat's PG DDL failed with "could not extend file… No space left on device"). Diagnosis: `/root/.vscode-server` had accumulated **16G** of Remote-SSH leftovers (8× ~490MB copies of one extension, ~15 stale Claude Code versions, 3.2G VSIX cache, 6 server builds). Cleaned strictly version-stale artifacts, keeping the running server build + running/newest extensions → **100% → 60%, 15G free**. All live services and the production DB verified healthy afterwards (`systemctl` + live-DB probe). The interrupted behat site re-initialised cleanly. Reported to the user for awareness: this is Remote-SSH churn and will regrow slowly; a periodic cleanup or `rm -rf ~/.vscode-server/data/CachedExtensionVSIXs` cron is worth considering.
