...
- **A7 — Concurrency control:** every race-sensitive gate (invitation acceptance, guide approval, succession confirm, staged-move commit, freeze/unfreeze, auto-grouping) runs inside `$DB->start_delegated_transaction()` **plus** a `\core\lock` named lock on the group (`mod_selfselectadvanced_group:{id}`) or activity (`...activity:{id}` for auto-grouping/move batches), re-validating all counts after acquiring the lock. Moodle-native, identical on both DBs.
