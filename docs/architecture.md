...
**Still open, stated rather than glossed:** `:overriderules` declares **both** `archetypes` and `clonepermissionsfrom => ':override'`, and core treats them as mutually exclusive (`accesslib.php`: *"we ignore archetype key if we have cloned permissions"*). Measured, both engines: a fresh install gives it to `editingteacher` + `manager`; the simulated upgrade gives it to `editingteacher` + this plugin's own **Group Coordinator** role — a non-editing-teacher role — at system context, and not to the manager. Three holder sets for one version, for a `RISK_CONFIG | RISK_DATALOSS` capability. Choosing ONE mechanism (and deciding deliberately whether a coordinator may park, dissolve and bypass) is a live decision, not a documented behaviour; §6.3a's claim that the two mechanisms are additive is wrong until it is taken.
