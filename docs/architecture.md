...
- **U3 — Selector search:** candidate selection must match on **first name, last name, or email** in every user selector. **[SUPERSEDED 2026-08-02, 1.20.1 wave 3D — the email half is WITHDRAWN in both states of the contact-privacy switch; PARTICIPANT selectors match names only. Rationale and the decision that replaced S7 are in A14 below.]** **[CARVE-OUT 2026-08-03/04, maintainer decisions 32 and 41 — the GUIDE selectors (`guides::with_load()`, the one seam every guide picker funnels through) match a guide's name, and their email address as well **when and only when the typed text contains `@`**; a guide is staff being approached, not a protected participant, and nothing renders the address. Never on username (decision 48). The participant selectors above are untouched, in both switch states. See A14's closing paragraph.]**
...
- **A7 — Concurrency control:** every race-sensitive gate (invitation acceptance, guide approval, succession confirm, staged-move commit, freeze/unfreeze, auto-grouping) runs inside `$DB->start_delegated_transaction()` **plus** `\core\lock` named locks taken through `\mod_selfselectadvanced\local\locks`, re-validating all counts on rows read *after* acquiring them. Moodle-native, identical on both DBs. There is **one** global lock order, written in code as `locks::ORDER` and repeated here verbatim:
  ...
  **How much of this is actually enforced, measured rather than asserted (corrected 2026-07-31 after the wave-1 audit — the previous wording claimed two things the tree does not have):**
  ...
  - *Ordering.* `locks::acquire()` reports a violation at `DEBUG_DEVELOPER` and nothing more. It does **not** fail a build by itself: Moodle turns an unconsumed `debugging()` into an `E_USER_NOTICE`, and PHPUnit only fails on that when it is run with `--fail-on-notice`. **That flag is NOT in the GitHub workflow, and this paragraph used to claim it was** (external audit FCA-002, 2026-08-13). `.github/workflows/moodle-ci.yml` runs `moodle-plugin-ci phpunit --fail-on-warning` and carries an explicit comment saying why `--fail-on-notice` is absent: the Plugin CI wrapper does not expose it. So the repository-wide backstop this paragraph promised does not exist, and a fork's CI does not enforce it. What DOES enforce it is the maintainer's own gate, which invokes PHPUnit directly with both flags (`/srv/ci/ops/ci-plugin-par.sh`) — a property of one machine, which is exactly what the old wording claimed to have escaped. So an unconsumed ordering violation turns a run red wherever the suite is run from this repository's own configuration. In production `debugging()` is a no-op at any level below DEBUG_DEVELOPER, so ordering is a development-time discipline, not a runtime guarantee. Any test that means to pin an ordering or no-mail property must still say so with an explicit `assertDebuggingCalled()` / `assertDebuggingNotCalled()`; do not rely on the ambient flag alone.
...
- **A14 — Selector matching and address display (U3; S7 REVERSED by wave 3D):** the candidate search WHERE clause matches the **full core name-field set** (`\core_user\fields::for_name()` — first/last/middle/alternate, mirroring core participants search [S6]) **and nothing else — never the email address, for any viewer, in either state of the contact-privacy switch** (1.20.1 wave 3D, P-5). MATCHING and DISPLAYING are two questions with two different answers: the address is still *appended to a label* when the switch is OFF and the viewer holds `moodle/site:viewuseridentity` or `moodle/course:viewhiddenuserfields` in course context, per core identity-field rules — showing the address of somebody found BY NAME discloses, it does not confirm a guess. **The S7 decision of 2026-07-24 (matching for all inviters, residual disclosure accepted) is withdrawn.** What it accepted as residual is an address ORACLE: a picker that answers found/not-found for a submitted address confirms which named account owns it, one query at a time, whether or not anything is rendered back — available to a student leader and invisible to any review. A per-activity setting decides what an activity *displays*; it was never a decision about what may be *probed*, so gating the match on the switch (or on the viewer) was the same mistake at two strengths. The UI follows: the invite placeholder is unconditionally "Search by name" (`inviteplaceholdername`), because a box must not promise a match the query will not make. Pinned by `candidateaddress_test`, `external_search_test::test_email_match_gated_when_private` (both switch states, service **and** web service), `contactreach_test` (P-5) and, at browser level, `invitations.feature` "An email address finds nobody in the native selector" (switch off) + `contactprivacy.feature` "The invite picker promises only a name search and matches only names" (switch on). The two staff CSV imports (`coordinatorimport::find_user()`, `attributes\csv_importer`) still resolve an operator-supplied address by exact equality and deliberately do not consult the switch — there the address is an input the operator already holds.
  **A SECOND, ORTHOGONAL EXCEPTION (maintainer decisions 32 and 41, 2026-08-03/04), and it is about the SUBJECT rather than the viewer.** Everything above is a rule over a pool of PARTICIPANTS, and its whole force is that an address probe against protected people is an oracle. The **guide** pickers query a different pool — the holders of `mod/selfselectadvanced:guide` in that module context, i.e. staff being deliberately approached — and there the maintainer ruled the other way: *"Guides are not a protected class."* So `guides::with_load()` matches the typed text against a guide's name, **and against their email address when and only when that text contains an `@`** (decision 41), for every viewer entitled to use a guide picker, in both states of the contact-privacy switch. The VIT journey is the reason: a student approaches a faculty member in person and comes away with an address or an employee id, and the employee id is recorded as the surname and therefore already matched.
  **SUPERSEDED 2026-08-13 (external audit DOC-002). THIS PARAGRAPH DESCRIBED A MATCHER THE CODE NO LONGER HAS, and it described it as a known, accepted oracle — which is the worst possible thing for a normative privacy document to be wrong about. The historical account, kept because the measurement was real: the address arm once engaged on any query containing `@`, a blind audit reconstructed a whole guide address in **453** `search_guides` calls by growing a substring on found/not-found alone, and the maintainer accepted that residue ('Option B: … @ slows deliberate probe'), with exact equality recommended and not taken. **What the code does today** (`guides.php`, `with_load()`): the address arm engages only when the whole query is a syntactically valid address, tested with core's `validate_email()`; the comparison is case-insensitive **equality**, never substring; a partial address does not engage the address arm at all and falls through to name matching; and `u.email` is SELECTED only when the query can use it, so a column that is never fetched cannot be leaked by a later edit. The 453-call oracle is therefore closed, not accepted — equality returns nothing for a prefix, so there is no found/not-found gradient to climb. Username is still never a match key (decision 48). Pinned by `guidepickeraddress_test`.
...
| `mod/selfselectadvanced:lead` | module | student | clones `:creategroup` on upgrade |
...
| `:lead` (source `:creategroup`, **1.20.26**) | student (archetypes) | every role holding a recorded `:creategroup` permission, with that same ALLOW, PREVENT or PROHIBIT | the upgrade itself changes no existing policy; after it, an administrator may prohibit `:creategroup` alone to pause new groups while existing leaders retain `:lead` |
...
| T2 | `forming` → `pending_guide` (submit) | leader `:lead` | window; **L1** met (confirmed ≥ eff. minsize); **quota** compliant or exempt; leader-selects: chosen guide has **L5** free (list pre-filtered, §4A.5) | timesubmitted; guideid set (or A5 queue); notify guide/manager; `group_submitted` |
...
| T6 | `frozen` → `firm` (unfreeze) | `:unfreeze` | warning if core group referenced by restrictions (list shown) | core group RETAINED (see the Unfreeze section below - the table said 'deleted' until 2026-08-13, external audit DOC-001); roster restored to latest snapshot; out-of-band drift discarded+reported; `group_unfrozen` |
| T7 | `forming` → *(deleted)* | leader `:lead` | forming only | notify members; cancel invitations; hard delete (A10); `group_deleted` |

Membership sub-machine (member rows): `invited → confirmed | declined | expired`; `confirmed → removed` (leave confirmed by leader; staged move; privacy delete). Succession sub-flow (A3) rides on the group row. `firm`: membership immutable except staged moves **(except a team its own guide has RELEASED - decision 56; corrected 2026-08-13, external audit LIFE-001)** (§7); `frozen`: additionally mirrored; **students and guides can never alter firm/frozen membership **(except a team its own guide has RELEASED - decision 56; corrected 2026-08-13, external audit LIFE-001)**** (§5).
...
| U3 | search by first/last name; **email matching withdrawn** on PARTICIPANT selectors (wave 3D, A14) | `search_candidates` (A14) | `candidateaddress_test`, `external_search_test`, `contactreach_test`, `guidepickeraddress_test` (the participant half of the boundary); behat: `invitations.feature` "An email address finds nobody in the native selector", `contactprivacy.feature` "The invite picker promises only a name search and matches only names" |
| U3 / decisions 32, 41, 48 | GUIDE selectors match name, plus email address **only when the query contains `@`**; never username; address never rendered and only fetched when matchable (A14 closing paragraph) | `guides::with_load()`, `search_guides` | `guidepickeraddress_test` |
